...
<section id="down" class="login-area sec-p">
    <div class="container">
        <div class="login-form">
            <h3>@lang('Log In')</h3>
            <span>@lang('New Member?')<a href="{{ route('user.register') }}">@lang('SignUp here')</a></span>
            <form id="loginform" action="{{ route('user.login.submit') }}" method="POST">
                @includeIf('includes.user.form-both')
                @csrf
                <label for="email">@lang('Email')*
                    <input type="email" name="email" id="username" class="form-control" value="seller@gmail.com">
                </label>
            ...
            <table class="table table-light mt-2">
                <tbody>
                    <tr>
                        <td>@lang('Seller Login'):</td>
                        <td>seller@gmail.com</td>
                        <td>1234</td>
                    </tr>
                    <tr>
                        <td>@lang('Buyer Login'):</td>
